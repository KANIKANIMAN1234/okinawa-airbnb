...
| **photos** | 施設写真管理 | 写真のURL・表示順・説明文 |
| **blocked_periods** | 予約禁止期間 | 管理者が設定した予約禁止期間（LINEアプリ上でのみブロック） |
...
## 5.2 blocked_periods シート（予約禁止期間）

| 列名 | データ型 | 説明 | 例 |
| :--- | :--- | :--- | :--- |
| **periodId** | 文字列 | 期間の一意識別子 | BP1234567890 |
| **startDate** | 文字列（日付） | 開始日（YYYY-MM-DD） | 2026-07-01 |
| **endDate** | 文字列（日付） | 終了日（YYYY-MM-DD） | 2026-08-16 |
| **reason** | 文字列 | 理由（オプション） | メンテナンス |

**運用:**
- 管理者のみが予約禁止期間の追加・削除が可能。
- **重要:** 予約禁止期間はLINEアプリ上でのみブロックされ、Googleカレンダーには追加されない（Airbnbには同期されない）。
- 複数の期間を設定可能。期間が重複していても問題なし。
- カレンダー表示時に、この期間内の日付はグレーアウト（選択不可）として表示される。
