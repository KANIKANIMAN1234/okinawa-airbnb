...
 * 家族収支管理アプリ - API Backend (v6.0.0)

 */

const SPREADSHEET_ID = "1oZXdw5LvJQqfYc6_nWehQgP6B6kDHU6AuXawljtibXw";

const FOLDER_ID = "1RkQ9GF9teR76ufQ4lUvra2Nx90S3cR-Q";
...
  const values = sheet.getDataRange().getValues();
  ...
  return values.slice(1).reverse().map(row => ({
    ...
    imageUrls: row[6] ? row[6].split(",") : [], // カンマ区切りを配列に戻す

    lineId: row[7] || "", // H列: LINE ID
...
// checkAndRegisterUser, getUserList, doGet は前回のままでOK
...
function getUserList() {
  ...
  return values.slice(1).map(row => ({
    lineId: row[0],  // A列: LINE ID
    name: row[1]      // B列: 名前
  }));

}

function doGet() { return ContentService.createTextOutput("GAS API v6.0.0 active."); }
